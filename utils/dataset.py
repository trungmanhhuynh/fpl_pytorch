...
def parse_data_CV(data, split_list, input_len, offset_len, pred_len, nb_train):
    if type(split_list) != list:
        split_list = [split_list]   # e.g 0, 1, 2 ,3 for train_splits
    trajectories = data["trajectories"].astype(np.float32)
    splits = data["splits"]         # split index of each sample 
    ...
    idxs_split = reduce(add, [splits == s for s in split_list])  # true/false values
...
class SceneDatasetCV(dataset_mixin.DatasetMixin):
    ...
    def get_example(self, i):
        ...
        return X, Y, self.video_ids[i], self.frames[i], self.person_ids[i], \
           horizontal_flip
